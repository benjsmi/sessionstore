...
_.extend(CouchDbSessionStore.prototype, {
  ...
  all: function (callback) {
    //design_doc, view_name, query parameters, callback
    var options = this.options;
    var client = new (cradle.Connection)(options.host, options.port, options.options);
    var db = client.database(options.dbName);
    db.view('sessions/findAll', { key: this.collectionName }, function (err, docs) {
      var res = [];
  ...
  clearExpired: function (callback) {
    var options = this.options;
    var client = new (cradle.Connection)(options.host, options.port, options.options);
    var db = client.database(options.dbName);
    ...
    this.all(function (err, results) {
      ...
      async.each(results, function (item, clb) {
        var now = Date.now();
        var expires = Date.parse(item.expires);
        if (now > expires) {
          db.remove(item._id, item._rev, clb);
          total_expired++;
        }
      }, function (err) {
        if (callback) callback(err);
      });
      console.log("expired:" + total_expired + " out of " + results.length);
    });
    if(callback){
      callback(null);
    }
